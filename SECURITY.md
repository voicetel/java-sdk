...
<https://github.com/voicetel/java-sdk/security/advisories/new>).
...
- Fetch the key once via `POST /v2.2/account/apikey` (the only
  ...
  the result in a secret manager or environment, never in source
  control. Re-fetch after any password change; the prior key is
  invalidated server-side.
- The SDK builds on `java.net.http.HttpClient` with HTTP/2 enabled
  and the JDK's default TLS posture (TLS 1.2+, session resumption
  via the JSSE session cache). If you supply a custom `HttpClient`,
...
Out of scope: vulnerabilities in the `com.voicetel:voicetel-sdk`
artifact caused by a forked / vendored copy that has been modified.
